...
switch subject
    case "Andreas"
        disp("Andreas")
        load('trigno_andreas_121.mat')  % load data from delsys trigno 
        load("kick_andreas.mat")        % load data from Mr.Kick  
        pretrig = 4;
    case "Pegah"
        disp("Pegah")
        load("kick_pegah.mat")        % load data from Mr.Kick  
        load('trigno_pegah_105.mat')  % load data from delsys trigno
        pretrig = 6;
   otherwise
      error("Not a valid obtions for subject input")
end
...
x_axis = linspace(-pretrig, 10 - pretrig,  20000 ); % from kick
for sweep = 1:length(included_sweeps)
    close all; 
    %disp(sweep)
    y_forceG = kick.data{sweep, kick.trig_level}; 

    threshold = 1; 
    %threshold = mean(y_forceG(end/2:end)) + abs(mean(y_forceG(end/2:end))*0.50); 

    trig_positive = find(y_forceG > threshold == 1);
    trig_position_idx = trig_positive(1); % samples
    trig_position_sec(sweep) = x_axis(trig_position_idx);  % sec

    % figure; hold on 
    % plot(x_axis, y_forceG(:))
    % plot([x_axis(1), x_axis(end)],[threshold, threshold])
...
for sweep = 1:acq.sweeps
    acq.N.AG(sweep)   = length(trigno.data{sweep,trigno.left_foot_IMU}(:,2));
    acq.N.O(sweep)    = length(trigno.data{sweep,trigno.left_foot_quaternion}(:,2));
    acq.N.EMG(sweep)  = length(trigno.data{sweep,trigno.left_soleus}(:,2));
    acq.N.Kick(sweep) = length(kick.data{sweep,1});

    acq.time_axis.AG{sweep}   = trigno.data{sweep,trigno.left_foot_IMU}(:,1) + trig_position_sec(sweep);
    acq.time_axis.ori{sweep}  = trigno.data{sweep,trigno.left_foot_quaternion}(:,1) + trig_position_sec(sweep);
...
for sweep = 1:acq.sweeps
    ...
    % foot
    w = trigno.data{sweep, trigno.left_foot_quaternion}(:, trigno.quat.w);
    x = trigno.data{sweep, trigno.left_foot_quaternion}(:, trigno.quat.x);
    y = trigno.data{sweep, trigno.left_foot_quaternion}(:, trigno.quat.y);
    z = trigno.data{sweep, trigno.left_foot_quaternion}(:, trigno.quat.z);
...
for c = 1:3
    ...
    for i = 1:length(classes{c})
        ...
        stair_pos(i,:) = kick.data{classes{c}(i),kick.pos_stair}; 
...
xlimits_ground = [-4 6.5]; 
xlimits_ground = [-1 1]; 
%xlimits_ground = [0.35 0.55]
%xlimits_ground = [0.25 0.55]
...
title("Left Soleus")
%plot(acq.time_axis.EMG{sweep}, trigno.data{sweep,trigno.left_soleus}(:,2), 'Color',"yellow",'LineWidth',1) 
...
% plot(acq.time_axis.EMG{sweep}, trigno.data{sweep,trigno.left_tibialis_anterior}(:,2), 'Color',"yellow",'LineWidth',1) 
